...
'-----------------------------------------------------------------------
'--- 上传处理类模块
'--- Copyright (c) 2005 team studio, Inc.
'--- Mail: teamserver@163.com   http://www.team5.cn
'--- 2006-09-13
'-----------------------------------------------------------------------
'-----------------------------------------------------------------------
...
Class UpFile_Cls
	...
	Private FilePath,InceptFile,FileMaxSize,MaxFile,Upload_Type,FileInfo,IsBinary,SessionName,TextName
	...
	Public ErrCodes,ObjName,UploadFiles,UploadForms,Count,CountSize,UpTextName
	'-----------------------------------------------------------------------------------
	'初始化类
	'-----------------------------------------------------------------------------------
	...
	End Property
	'-----------------------------------------------------------------------------------
	'设置表单类型属性 (以逗号分隔多个表单类型)
	'-----------------------------------------------------------------------------------
	Public Property Let ChkTextName(Byval Values)
		TextName = Values
	End Property
	'-----------------------------------------------------------------------------------
	'设置上传文件大小上限 (单位：kb)
	...
	Public Property Get Description
		Select Case ErrCodes
			...
			Case 8 : Description = "水印组件选择错误或服务器未安装此组件。"
			Case Else
				Description = Empty
		End Select
	End Property
	...
	Private Function CheckFileExt(FileExt)
		...
		If FileExt="" or IsEmpty(FileExt) Then
			...
			Exit Function
		End If
		If FileExt="asp" Or  FileExt="asa" Or FileExt="aspx" Or FileExt="php" Or FileExt="js" Then
			CheckFileExt = False
			Exit Function
		End If
	...
	Public Sub SaveUpFile()
		...
		Select Case (Upload_Type) 
			...
			Case Else
				ErrCodes = 2
		End Select
		UpTextName = UploadObj.Form(TextName)
	End Sub
